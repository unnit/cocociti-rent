...
$(document).ready(function () {
  ...
  $("html,body").click(function (e) {
    var container_1 = $(
      ".ps-wrapper, .alert, .sweet-overlay, .sweet-alert, .ps-nav-btn, .pac-input, .j-coco-fixed-wrapper",
    );
    if (
  ...
  $(document).on("change", "#showcase_accept_fund", function () {
    ...
    if ($(this).is(":checked")) {
      ...
      $wrap
        .find(".goal-target-date")
        .removeAttr("disabled")
        .attr("readonly", true);
      $wrap
        .find(".more-target-date")
        .addClass("hidden")
        .removeAttr("readonly")
        .attr("disabled", true);
      ...
      if ($wrap.find("#showcase_showcase_type").val() == 0) {
        date_options["startDate"] = new Date(
          $wrap.find(".ps-wrapper, .ps-edit-wrapper").data("pdate"),
        );
        $wrap.find(".dt-of-achievement").val("").datetimepicker("remove");
        $wrap
          .find(".dt-of-achievement")
          .datetimepicker(date_options)
  ...
  $(document).on("click", ".ps-wrapper", function (e) {
    ...
    if (
      $wrap.find(".type-holder").children().length == 0 &&
      $wrap.find(".prefix-holder").children().length == 0
    ) {
      $wrap
        .find(".ps-initial")
        .attr("placeholder", "Please select your wish type");
  ...
  function showPsNext() {
    if (
      $wrap.find("#showcase_goal_amount").val().trim() != "" &&
      $wrap.find("#showcase_target_date").val().trim() != "" &&
      $wrap.find("#showcase_fundcategory_id").val().trim() != "" &&
      $wrap.find(".ps-initial").val().trim() != "" &&
      (($("input[name='showcase[raising_for]']:checked").val() == 2 &&
        $("#showcase_beneficiary").val().trim() != "") ||
        $("input[name='showcase[raising_for]']:checked").val() == 1)
    ) {
  ...
  $(document).on("click", ".type-holder-dropdown li a", function () {
    ...
    $wrap
      .find(".type-holder")
      .css({ top: "32px", left: "5px", "font-size": "15px" });
    ...
    if ($wrap.find("#showcase_showcase_type").val() == 0) {
      ...
      date_options["startDate"] = new Date(
        $wrap.find(".ps-wrapper, .ps-edit-wrapper").data("fdate"),
      );
  ...
  $(document).on("click", ".prefix-holder-dropdown li a", function () {
    ...
    if ($wrap.find("#showcase_showcase_type").val() == 0) {
      $wrap
        .find(".ps-initial")
        .attr("placeholder", $(this).data("pplaceholder"));
      $wrap
        .find(".prefix-holder")
        .html(
          "<span class='dotted-bt-border mleft5'>" +
            $(this).data("ptext") +
            "&nbsp;<i class='fa fa-angle-down font16' aria-hidden='true'></i></span>",
        );
    } else {
      $wrap
        .find(".ps-initial")
        .attr("placeholder", $(this).data("fplaceholder"));
      $wrap
        .find(".prefix-holder")
        .html(
          "<span class='dotted-bt-border mleft5'>" +
    ...
    $wrap.find(".ps-initial").css({
      width:
        $wrap.find(".j-ps-holder").width() -
        $wrap.find(".prefix-holder").width() -
        3,
    });
  ...
  $(document).on("click", ".j-ps-more", function () {
    ...
    $wrap.find(".ps-sub-wrap-2").fadeIn(50, function () {
      $wrap
        .find(".ps-sub-wrap-2")
        .animate(
          { opacity: "1", "margin-left": "10px", "margin-top": "-120px" },
          150,
        );
    });
  });
  $(document).on("click", ".j-ps-holder, .ps-wrap-2-close", function (e) {
    ...
    if ($wrap.find(".ps-sub-wrap-2").css("margin-left") == "10px") {
      $wrap
        .find(".ps-sub-wrap-2")
        .animate(
          { "margin-left": "0px", "margin-top": "-130px" },
          150,
          function () {
            $wrap.find(".ps-sub-wrap-2").fadeOut(50, function () {
              $wrap.find(".ps-sub-wrap-2").css({ opacity: 0 });
            });
          },
        );
    } else {
      if ($(e.target).is(".ps-wrap-2-close")) {
        ...
        $wrap
          .find(".ps-sub-wrap-1 .showcase-submit")
          .removeClass("j-showcase-more-submit")
          .addClass("j-showcase-more-submit");
  ...
  function psSubmit(title) {
    if ($wrap.find(".preview, .preview-edit").children().length == 0) {
      if (
        $wrap.find(".j-new-wish").length &&
        $wrap.find("#showcase_description").val().trim() == "" &&
        $wrap.find(".more-target-date").val().trim() == "" &&
        $wrap.find("[name='showcase[all_tags]']").val().trim() == "" &&
        $wrap.find("#showcase_location_attributes_name").val().trim() == "" &&
        $wrap.find("#showcase_projected_amount").val().trim() == ""
      ) {
        $(".j-coco-fixed-wrapper").show();
        $wrap
          .find(".ps-sub-wrap-2 .showcase-submit")
          .removeClass("j-showcase-more-submit")
          .addClass("j-showcase-more-submit")
          .val("Skip & Wish");
  ...
  $(document).on("click", ".showcase-fullfillment-submit", function (e) {
    ...
    psSubmit(
      "Are you sure to post your fullfillment/achievement of your wish without a photo?",
    );
  });
  ...
  $(".cloudinary-fileupload-new").on("cloudinarydone", function (e, data) {
    ...
    $wrap.find(".preview").html(
      $.cloudinary.image(data.result.public_id, {
        ...
        width: 100,
        height: 100,
        class: "img-responsive inline-display",
      }),
    );
  ...
  $(document).on("click", ".coin-brd-box", function () {
    $(this)
      .closest(".coin-wish-wrapper")
      .find(".coin-fild-box")
      .removeClass("coin-fild-box")
      .addClass("coin-brd-box");
    $(this)
      .closest(".coin-wish-wrapper")
      .find(".fa-circle")
  ...
  $(document).on("click", ".rewish-link", function () {
    $("#cont-wrapper").html(
      "<div class='container padding0 font17' style='max-width:450px;margin-top: 10%;'><div class='col-xs-12 col-sm-12 bg-white padding20 border5 cc-dark-bg white-fg'><span class='full-width pull-left mbottom30'>Awesome! This will appear in your wishlist. You can edit the content after rewishing.</span><div class='col-xs-6 col-sm-3 col-sm-offset-6 padding10'><a href='/wish/" +
        $(this).data("id") +
        "/rewish' class='btn btn-sm bg-white cc-dark-fg full-width post-rewish' data-method='post'>Rewish</a></div><div class='col-xs-6 col-sm-3 padding10'><a class='btn btn-sm full-width white-fg light-border cancel-rewish'>Not now</a></div></div></div>",
    );
    $("#cont-wrapper").prepend(
      "<span class='pull-right padding5 mbottom20' style='z-index: 1051;'><button type='button' data-dismiss='modal' class='pull-left btn grey-bg padding10' style='border-radius: 50%;'><span class='close-sprite pull-left'></button></span>",
    );
  ...
  $(document).on("click", ".done-this", function () {
    $("#cont-wrapper").html(
      "<div class='container padding0 font17' style='max-width:450px;margin-top: 10%;'><div class='col-xs-12 col-sm-12 bg-white padding20 border5 cc-dark-bg white-fg'><span class='full-width pull-left mbottom30'>You are about to mark this wish as fulfilled.</span><div class='col-xs-6 col-sm-3 col-sm-offset-6 padding10'><a href='/wish/" +
        $(this).data("id") +
        "/have_done_this' class='btn btn-sm bg-white cc-dark-fg full-width post-have-done' data-method='post'>Proceed</a></div><div class='col-xs-6 col-sm-3 padding10'><a class='btn btn-sm full-width white-fg light-border cancel-done-this'>Not now</a></div></div></div>",
    );
    $("#cont-wrapper").prepend(
      "<span class='pull-right padding5 mbottom20' style='z-index: 1051;'><button type='button' data-dismiss='modal' class='pull-left btn grey-bg padding10' style='border-radius: 50%;'><span class='close-sprite pull-left'></button></span>",
    );
  ...
  $(document).on("click", ".search-mob", function () {
    $(
      ".feed-mob, .notif-mob, .follow-mob, .conv-mobile, .chat-room-mobile",
    ).removeClass("cc-med-bg");
    $(".navbar-header").hide();
  ...
  $(".scase-login").hover(
    function () {
      $(this).html(
        "<i class='fa fa-long-arrow-left' aria-hidden='true'></i>&nbsp;Login/SignUp",
      );
    },
    function () {
      $(this).text("Showcase");
  ...
  $(".username").on("keyup", function () {
    ...
    if ($(this).val().length > 5) {
      ...
      $.get(
        "/profiles/username_available",
        { uname: $(this).val() },
        function (data) {
          $(".uname-avail").text(data.result);
          if (data.result == "Available") {
            $(".uname-avail").append(
              "&nbsp;<i class='fa fa-check-circle green-fg font17'></i>",
            );
          }
        },
      );
  ...
  $("html,body").click(function (e) {
    var container = $(
      ".notif-icon, #mCSB_2_scrollbar_vertical, .web-notifications",
    );
    if (
  ...
  //Loader Icon
  $(
    "form.bulk-bookings-form, form.user_form, form.login-form, form.signup-form, form.form-wiki-wrap",
  ).submit(function () {
  ...
  $(document).on("click", ".share_popup_icon", function () {
    $(".share_popup")
      .not($(this).next(".share_popup"))
      .each(function () {
        $(this).hide();
      });
    $(".shareShowcaseRoundIcons").jsSocials({
      url: "https://www.cocociti.com/wish/" + $(this).attr("data-showcase-id"),
      showLabel: false,
      showCount: false,
  ...
  $(document).on("click", ".j-wishpay-open", function () {
    $(this)
      ...
      .fadeToggle(100);
    $(".j-wishpay-wrap")
      .not($(this).closest(".showcase-ps-wrapper").find(".j-wishpay-wrap"))
      .each(function () {
        $(this).hide();
      });
  });
  ...
  $(document).on("click", ".add-move-showc", function () {
    ...
    $(this)
      .closest(".move-show-main-wrapper")
      .find(".move-create-coll-wrapper,.create-collect")
      .show();
  });
  ...
  $(document).on("click", ".gway-rqst", function () {
    $(this).html(
      "<span>Please Wait...</span><span class='ball-clip-rotate' style='height:22px;'><div style='height:15px;width:15px;'></div></span>",
    );
  });
  ...
  $(document).on("click", ".embed-link", function () {
    $("#cont-wrapper").html(
      "<div class='modal-dialog'><div class='modal-content pop-up-mar'><div class='modal-header pop-bac'><button class='close' data-dismiss='modal' type='button'> <i class='fa fa-times'></i></button><h4 class='modal-title'>Embed</h4></div><div class='modal-body'><div class='col-sm-12 col-xs-12'><h5 class='weight600'>Embed this Showcase to your website by copying the code below</h5><textarea rows='4' class='full-width grey-fg' onClick='this.setSelectionRange(0, this.value.length)' readonly=true><a class='embedly-card' href='" +
        $(this).data("url") +
        "'>" +
        $(this).data("title") +
        "</a><script async src='https://cdn.embedly.com/widgets/platform.js' charset='UTF-8'></script></textarea><br><br></div></div><div class='clearfix'></div></div></div>",
    );
    $("#cont-wrapper").modal("show");
  });
  ...
  if ($("#infinite-scrolling-content").length) {
    $(window).on("scroll", function () {
      var more_posts_url = $(
        "#infinite-scrolling-content .pagination .next a",
      ).attr("href");
      if (
        more_posts_url &&
  ...
  $(".rent-hover").hover(function () {
    ...
    $("#rent-lend-wrapper").hasClass("show-rent")
      ? ($(".arrow-rent").addClass("clockwise180"),
        $(".home-rent-icon span").addClass("opacity"),
        setTimeout(function () {
          $(".rent-lend-content").addClass("mtop60");
        }, 400))
      : ($(".arrow-rent").removeClass("clockwise180"),
        $(".home-rent-icon span").removeClass("opacity"),
        $(".rent-lend-content").removeClass("mtop60"));
  });
  $(".lend-hover").hover(function () {
    ...
    $("#rent-lend-wrapper").hasClass("show-lend")
      ? ($(".arrow-lend").addClass("clockwise180"),
        $(".home-lend-icon span").addClass("opacity"),
        setTimeout(function () {
          $(".rent-lend-content").addClass("mtop55");
        }, 400))
  ...
  if ($("#adv-search").length) {
    $.get("/products/sub_categories", {
      category: $("#category").val(),
      selected: $("#filter-sub-cat").data("selected"),
    });
  }
  ...
  $(".available-switch").click(function () {
    ...
    $(".update-msg").html("");
    $.post(
      "/products/update_available",
      { id: $(this).data("id"), available: available },
      function (result) {
        if (result["error"]) {
  ...
  search_start_date = $("#start_date_and_time_for_search_datepicker").data(
    "id",
  );
  ...
  $(".land-src-button").click(function () {
    ...
    setTimeout(function () {
      $(".home_start_date").attr("readonly", true);
    }, 1000);
    setTimeout(function () {
      $(".home_end_date").attr("readonly", true);
    }, 1000);
  });
  ...
  $(".featured-toggle").click(function () {
    ...
    $.post(
      "/admin/products/" + $(this).data("id") + "/set_featured",
      {},
      function (result) {
        if (result["error"]) {
          $closest.find(".update-msg").html("");
  ...
  $(
    "#booking_requests_table, #my_listings_table, #my_orders_table, #upcoming_bookings_table, #non_coco_bookings_table, #delete_non_coco_bookings_table",
  ).tablesorter();
    ...
    if (authorizationResult && !authorizationResult.error) {
      $.get(
        ...
        function (response) {
          ...
          if ($(".import-emails").val().length == 0) {
            $(".status-invite").html(
              "<span style='color: red;'>Sorry, No emails found.</span>",
            );
          }
        },
      );
  ...
  $(document).on(
    "swiperight",
    "#myCarousel, #myMobileCarousel, #myCoinCarousel, #myCoinMobileCarousel, .description_carousel, .image_carousel, #myCategoryCarousel",
    function () {
      $(this).carousel("prev");
      $(this)
        .find(".j-change-count")
        .text($(this).find(".j-change-count").text() === "1" ? "2" : "1");
  ...
  $(document).on("click", "a.j-enable-notification", function () {
    askPermission();
  });
  $(document).on("click", ".alert-close-notification", function () {
    $(this)
      .closest(".j-enable-web_notification, .j-how-to-enable-notifications")
      .addClass("hidden");
  });
  $(document).on("click", "a.j-enable-notification-instructions", function () {
    $(".j-enable-web_notification, .j-how-to-enable-notifications").addClass(
      "hidden",
    );
    content =
      "<div class='col-xs-12 col-sm-6 col-sm-offset-3 mtop40 bg-white padding20 border5 font11'> <h3 class='full-width mbottom10 text-center txt-underline font16'>Allow or block browser notifications Chrome</h3> <ul> <li class='mbottom5'>At the top right, click More More and then Settings.</li> <li class='mbottom5'>At the bottom, click Advanced.</li> <li class='mbottom5'>Under 'Privacy and security,' click Content settings.</li> <li class='mbottom5'>Click Notifications.</li> <li class='mbottom5'>Search 'https://www.cocociti.com' & choose to block or allow notifications: </li> </ul> <h3 class='full-width mbottom10 text-center txt-underline font16'>Allow or block browser notifications FireFox</h3> <ul> <li class='mbottom5'>Go to the Firefox menu and select Preferences.</li> <li class='mbottom5'>Select the Content panel and click the Choose… button under Notifications.</li> <li class='mbottom5'>Select 'https://www.cocociti.com'. </li> <li class='mbottom5'>Click Remove Site.</li> </ul> <br><br> <strong>P.S:</strong> If you’re browsing in Incognito mode, you won’t get notifications. <br><br><strong>Keep Wishing.</strong> </div>";
    display_cont_wrapper(content);
  });
  $("form#admin_send_firebase_notifications")
  ...
  $(document).on(
    "keyup",
    "form.new_cocotransfer #cocotransfer_total_amount",
    function (e) {
...
function appendLoader(thisLinkPreview) {
  $(thisLinkPreview).after(
    "<div class='link-preview-loader'><i class='fa fa-spinner fa-spin font22'></div>",
  );
}
...
function setFullfillmentOrContribute() {
  ...
  $fullfillmentAmount = parseInt(
    $("#cocotransfer_fullfillment_at_once_amount").val(),
  );
  console.log(
    "current:" +
      $currentAmount +
  ...
  if ($("#cocotransfer_transferable_type").val() == "Showcase") {
    if ($currentAmount == $fullfillmentAmount) {
      setWishFullfillment();
    } else {
      setWishContribute();
    }
...
function setOnlineAndWalletAmount() {
  $currentAmount = parseInt($("#cocotransfer_total_amount").val());
  $availableWalletAmount = parseInt(
    $("#cocotransfer_available_wallet_amount").val(),
  );
  if ($("#cocotransfer_use_wallet_amount").is(":checked")) {
    if ($availableWalletAmount >= $currentAmount) {
      ...
      $("#cocotransfer_amount_display").html(
        $currentAmount - $availableWalletAmount,
      );
...
function starRating(elementselector) {
  $rateYo = $(elementselector).rateYo({
    ...
    onInit: function (rating, rateyo) {
      $(this).rateYo("option", "ratedFill", customRatingColor(rating));
      $($(this).attr("data-display-rating")).html(rating);
    },
    onSet: function (rating, rateyo) {
      color = customRatingColor(rating);
      $(this).rateYo("option", "ratedFill", customRatingColor(rating));
      $(this)
        .closest("form")
        .find($(this).attr("data-input-field"))
        .val(rating);
      $($(this).attr("data-display-rating")).html(rating);
      $(this)
        .closest("form")
        .find($(this).attr("data-display-rating-map"))
        .html(ratingMapWord(rating));
      if ($(this).data("save-rating")) {
        save_rating($(this), rating);
      }
    },
    onChange: function (rating, rateyo) {
      color = customRatingColor(rating);
      $(this).rateYo("option", "ratedFill", customRatingColor(rating));
      $($(this).attr("data-input-field")).val(rating);
      $($(this).attr("data-display-rating")).html(rating);
    },
  });
}
...
function replace_content(element, modal, form) {
  form
    .find("#showcase_date_of_achievement")
    .val(element.attr("data-date_of_acheivement"));
  form
    .find("#showcase_achieved_description")
    .val(element.attr("data-showcase_achieved_description"));
}
    ...
    if ($preview.data("source") == "showcase") {
      ...
      $preview.html(
        $.cloudinary.image(data.result.public_id, {
          ...
          version: data.result.version,
          crop: "fill",
          height: $(this).data("height-non-source"),
          class: "img-responsive inline-display",
        }),
      );
    }
...
var messages_to_bottom = function () {
  return $("#chat_messages").scrollTop(
    $("#chat_messages").prop("scrollHeight"),
  );
};
...
var informServerAboutBlock = function (currentToken) {
  $.ajax({
    ...
    url: "/home/save_firebase_token_status",
    success: function (data) {
  ...
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker
      ...
      .then(function (registration) {
        ...
        if (!firebase.messaging().registrationToUse_) {
